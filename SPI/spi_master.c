...
void spi_func_init(void){
	...
	RCC_SSI0_CLK_ENABLE();
	spi_conf_t spi_master_conf;
	spi_handle.Init = spi_master_conf;
	spi_handle.Init.Clock = SPI_SYS_CLK;
	...
	spi_handle.Init.Clock_prescale = 64;
	spi_handle.Init.Loopback = SPI_LOOPBACK;
	spi_handle.Init.Mode = SPI_MASTER_SEL;
	spi_handle.Init.SCR = 0;
...
int main(void){
	...
	while(1){
		// Master blinks signal
		uint8_t master_signal[3] = {'A', 'B', 'C'};
		blink_pattern(master_signal, LED_RED);
		// Master transmits signal
		while(spi_status_check(&spi_handle, SPI_BUSY));
		spi_master_tx(&spi_handle, master_signal, 3);
		while(spi_status_check(&spi_handle, SPI_BUSY));
		// Wait for the slave
		delay(10);
		...
		while(spi_status_check(&spi_handle, SPI_BUSY));
		spi_master_rx(&spi_handle, master_receive, 3);
		while(spi_status_check(&spi_handle, SPI_BUSY));
		blink_pattern(master_receive, LED_RED);
	}
}
